...
* Name: mtv4
* Author: Lina
* Description: Basic simulation model 
* Tags: Tag1, Tag2, TagN
***/
...
global {
    ...
    //building polygons
    file building_polygons <-file("../includes/gleisdorf_buildings_complete.shp");
...
//    	create dh_potential from: dh_potential_gleisdorf;
	...
	reflex save_buildings_2035 when: cycle = 18{
		ask buildings {
			...
			save buildings to: "../results/building_agents_2035.csv" type:"csv" rewrite: false;
...
//				reflex shp_pot_check{
//					ask buildings at_distance dist {				
//						if (myself.potential = 1) and (self.inside_pvp = true) {
//						self.inside_solep <- true;
//							}		
//					}		
//				}								
	}
	...
	species water_hp_potential parent:potentials topology: topology(water_hp_potential_gleisdorf){		
				...
				reflex whp_pot_check{
			ask buildings at_distance dist {
				if (myself.potential = 1) and (self.inside_pvp = true) {
					self.inside_waterp <- true;
					}		
			}		
		}				
	}
	...
	species biomass_potential parent:potentials topology: topology(biomass_potential_gleisdorf){
				rgb color <- #olivedrab;
				reflex bio_pot_check{
			ask buildings at_distance dist {
				self.inside_biomassp <- true;
			}		
		}						
	}
	...
	species oil_potential parent:potentials topology: topology(oil_potential_gleisdorf){
				rgb color <- #dimgrey;
				reflex oil_pot_check{
			ask buildings at_distance dist {
				self.inside_oilp <- true;
			}		
		}						
	}
...
//BEHAVIOURS//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	reflex renovation_behaviour when: is_renovated = false{
		...
		if residential_type = "nonres" {
			...
			if building_age<41{		
				renovation_status_actual <- "no need for renovation";
				write "Building No: " + name + " renovation at: " + cycle;
				write "---------------------------------------------------";
...
//	//   !!!!!!!!!!!!!!!!!!!!!!<Work in Progress>!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	
		...
		else if (inside_solep = true) /* and (inside_pvp = true)*/  {
			
			heating_system_actual <- "sole_hp";
			}
		else if (inside_waterp = true) /*  and (inside_pvp = true)*/ {
			
			heating_system_actual <- "water_hp";
			}
		else if (inside_airp = true)/*  and (inside_pvp_1 = true)*/ {
...
//action of building renovation		
	action renovation_action{
		...
		reflex update_status {
			//fill a list with statistical data?
