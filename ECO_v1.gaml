...
* Name: ECO
* Author: Lina Stanzel
* Description:	1.3.	Economic (ECO) Scenario for energy networks in Gleisdorf
...
global {
    ...
    init {
    	...
    	save  ("-------------------------------------------------------------------------")  to: "../results/results_ECO.txt" rewrite: false;
    		}	
	...
	//safe statistics in .txt file
    	reflex save_result when: every (1#cycles) {
			...
			("cycle : "+ (cycle) + " no need for renovation : " + (no_need_count)
	   		...
	   		to: "../results/results_ECO.txt" rewrite: false;
		}

	//safe total heb statistics as csv	
		reflex save_heb_development when: every (1#cycles) {			
			save("cycle : "+ (cycle) + " heb-list: ") to: "../results/total_heb_development_ECO.csv" type:"csv" rewrite: false;
				ask buildings {
					save [total_heb_actual] to: "../results/total_heb_development_ECO.csv" type:"csv" rewrite: false;
				}
			save("--------------------------------------------------------" ) to: "../results/total_heb_development_ECO.csv" type:"csv" rewrite: false;
		}
...
//			save buildings to: "../results/building_ECO_2035.csv" type:"csv" rewrite: true;
//			//Shapefile for Display 
//			//save buildings to: "../results/building_ECO_2035_.shp" type:"shp" attributes: ["ID"::int(self)];
//		}
	...
	reflex save_buildings_2050 when: cycle = 33{ 
		ask buildings {
			...
			// save all the attributes values of the bug agents in a file. The file is overwritten at every save
			//save buildings to: "../results/building_ECO_2050.csv" type:"csv" rewrite: true;
		}		
		do pause;
...
//BEHAVIOURS//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	reflex heating_system_update{
		...
		if is_renovated = true {
			...
			else if inside_oilp = true {
				heating_system_actual <- "oil";
			}
			else if (inside_airp = true) {		
			heating_system_actual <- "air_hp";		
			}
			else if inside_dhp = true {			
...
//action of building renovation		
	action renovation_action{
		...
		is_renovated <- flip(rnd(0.04, 0.12));
